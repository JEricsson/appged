...
export class LibraryView extends MWCView {
  ...
  protected createHeader(): void {
    ...
    this.MWCViewElements.headerContainer.innerHTML = `
      ...
      <div class="mwc-view-header-actions">
        <button type="button" class="mwc-library-header-btn selectAll">Selecionar tudo</button>
        <button type="button" class="mwc-library-header-btn cancel">Cancelar</button>
  ...
  protected updateEmptyContentHTML() {
    ...
    <div class="mwc-default-empty-library">
      ...
      <div class="desc">
        <div>
          Opção 1: Clique em "<b>Novo</b>" para criar um documento em branco
        </div>
        <div>
          Opção 2: Clique em "<b>Capturar</b>" ou "<b>Importar</b>" para usar imagens
        </div>
      </div>
    </div>
    `;
  ...
  private handleRename(docId: string) {
    ...
    showModal({
      ...
      title: "Renomear documentot",
      placeholder: "Insira o nome do documento",
      initialValue: doc.name,
      confirmText: "Renomear",
      validation: (value) => {
        if (value.length < 1) {
          return "O nome não pode estar vázio";
        }
        return null;
      },
  ...
  protected createToolbars(): void {
    ...
    const normalButtons = [
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.uploads?.label || "Envios",
  ...
  private async handleDelete() {
    const confirmed = await showModal({
      ...
      message: `Tem certeza de que deseja excluir ${this.checkedDocUids.length} document(s)?`,
      confirmText: "Delete",
      cancelText: "Cancel",
      showCloseBtn: true,
    });
