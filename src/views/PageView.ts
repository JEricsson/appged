...
export class PageView extends MWCView {
  ...
  protected createToolbars(): void {
    ...
    const canSharePDF = "Compartilhar" in navigator && navigator.canShare({ files: [testPdfFile] });
    ...
    const buttons = [
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.delete?.label || "Excluir",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.addPage?.label || "Adicionar página",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.share?.label || (canSharePDF ? "Compartilhar" : "Baixar"),
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.upload?.label || "Carregar",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.edit?.label || "Editar",
    ...
    const editButtons = [
      {
        ...
        label: toolbarButtonsConfig?.crop?.label || "Cortar",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.rotate?.label || "Girar",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.filter?.label || "Filtro",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.annotate?.label || "Anotar",
      ...
      },
      {
        ...
        label: toolbarButtonsConfig?.done?.label || "Concluir",
  ...
  private async handleDeletePage(): Promise<void> {
    const confirmed = await showModal({
      ...
      message: "Tem certeza de que deseja excluir esta página?",
      confirmText: "Excluir",
      cancelText: "Cancelar",
      showCloseBtn: true,
    });
  ...
  private async handleUpload(): Promise<void> {
    if (!this.config?.exportConfig?.uploadToServer) {
      await showToast(this.config.container, "Nenhuma função de upload configurada", ModalVariant.WARNING);
      return;
    }
    ...
    try {
      ...
      await showToast(this.config.container, "Falha ao carregar a página", ModalVariant.ERROR);
    }
  }
  ...
  private async handleEditMode(): Promise<void> {
    try {
      ...
      if (!status) throw new Error("Falha ao atualizar o visualizador de edição");

      this.toggleSelectionMode(true);
    } catch (ex) {
      await showToast(this.config.container, "Falha ao entrar no modo de edição", ModalVariant.ERROR);
      console.error(ex);
    }
  }
  ...
  private async handleDoneBtn(): Promise<void> {
    try {
      const confirmed = await showModal({
        ...
        message: "Você pode ter alterações não salvas. Deseja salvar antes de sair?",
      ...
      if (!status) {
        throw new Error("Falha ao atualizar o visualizador de edição");
      }

      this.toggleSelectionMode(false);
      this.enableAllEditButtons();
    } catch (ex) {
      await showToast(this.config.container, "Falha ao sair do modo de edição", ModalVariant.ERROR);
      console.error(ex);
    }
  }
